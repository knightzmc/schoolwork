...
class Location:
    def __init__(self):
        self.Fox: Fox = None
        self.Warren: Warren = None
        self.Den: Den = None


class Simulation:
    __Landscape: [[Location]]
    ...
    def __init__(
        ...
        self.__CreateLandscapeAndAnimals(
            InitialWarrenCount, InitialFoxCount, self.__FixedInitialLocations
        )
        ...
        while (self.__WarrenCount > 0 or self.__FoxCount > 0) and MenuOption != 5:
            ...
            if MenuOption == 4:
                ...
                if not self.__Landscape[x][y].Warren is None:
                    ...
                    self.__ViewRabbits = input(
                        "View individual rabbits (y/n)? ")
                    if self.__ViewRabbits == "y":
                        self.__Landscape[x][y].Warren.ListRabbits()
            if MenuOption == 6:
                x, y = self.find_biggest_warren()
                print('Biggest Warren at (' + str(x) + ', ' + str(y) + ')')
            if MenuOption == 7:
                self.inspect_all_rabbits()
        input()
    ...
    def inspect_all_rabbits(self):
        rabbits = []
        for x in range(0, self.__LandscapeSize):
            for y in range(0, self.__LandscapeSize):
                warren: Warren = self.__Landscape[x][y].Warren
                if warren is not None:
                    for rabbit in warren.get_rabbits():
        ...
        rabbits = reversed(bubble_sort(
            rabbits, lambda rabbit: rabbit.get_age()))
        for rabbit in rabbits:
            rabbit.Inspect()

    def find_biggest_warren(self):
        biggestWarren = None
        biggestCoords = (-1, -1)
        for x in range(0, self.__LandscapeSize):
            for y in range(0, self.__LandscapeSize):
                warren: Warren = self.__Landscape[x][y].Warren
                if biggestWarren is None or warren is not None and warren.GetRabbitCount() > biggestWarren.GetRabbitCount():
                    biggestWarren = warren
                    biggestCoords = x, y
        ...
        for x in range(0, self.__LandscapeSize):
            for y in range(0, self.__LandscapeSize):
                if not self.__Landscape[x][y].Warren is None:
                    ...
                    self.__Landscape[x][y].Warren.AdvanceGeneration(
                        self.__ShowDetail)
                    if self.__ShowDetail:
                ...
                den = self.__Landscape[x][y].Den
                if den is not None:
                    den.AdvanceGeneration(self.__ShowDetail)
                    if self.__ShowDetail:
                        print("Den at (", x, ",", y, "): ", sep="")
                    if den.should_spawn_new_fox():
                        den.foxes_created += 1
                        self.__CreateNewFox(True)

        if NewFoxCount > 0:
            if self.__ShowDetail:
                print("New foxes born: ")
            for f in range(0, NewFoxCount):
                self.__CreateNewFox(False)
        if self.__ShowDetail:
            input()
        self.__DrawLandscape()
        print()

    def __CreateLandscapeAndAnimals(
        self, InitialWarrenCount, InitialFoxCount, FixedInitialLocations
    ):
        for x in range(0, self.__LandscapeSize):
            for y in range(0, self.__LandscapeSize):
        ...
        if FixedInitialLocations:
            ...
            self.__Landscape[11][4].Warren = GiantWarren(
                self.__Variability, 115)
    ...
    def __CreateNewFox(self, override):
        x = random.randint(0, self.__LandscapeSize - 1)
        y = random.randint(0, self.__LandscapeSize - 1)
        while not override and not self.__Landscape[x][y].Fox is None:
            x = random.randint(0, self.__LandscapeSize - 1)
            y = random.randint(0, self.__LandscapeSize - 1)
        if self.__ShowDetail:
    ...
    def __FoxesEatRabbitsInWarren(self, WarrenX, WarrenY):
        RabbitCountAtStartOfPeriod = self.__Landscape[WarrenX][
            WarrenY
            ...
            for FoxY in range(0, self.__LandscapeSize):
                if not self.__Landscape[FoxX][FoxY].Fox is None:
                    ...
                    if Dist <= 3.5:
                        ...
                        PercentToEat = 0
                    RabbitsToEat = int(
                        round(float(PercentToEat * RabbitCountAtStartOfPeriod / 100))
                    )
                    FoodConsumed = self.__Landscape[WarrenX][WarrenY].Warren.EatRabbits(
                        RabbitsToEat
                    )
            ...
            for x in range(0, self.__LandscapeSize):
                if not self.__Landscape[x][y].Warren is None:
                    if self.__Landscape[x][y].Warren.GetRabbitCount() < 10:
                        print(" ", end="")
                    print(self.__Landscape[x]
                          [y].Warren.GetRabbitCount(), end="")
                else:
                    print("  ", end="")
                if self.__Landscape[x][y].Den is not None:
                    print("D", end="")
                    print(self.__Landscape[x][y].Den.foxes_created, end="")
                else:
                    print("  ", end="")
                if not self.__Landscape[x][y].Fox is None:
                    print("F", end="")
...
class Warren:
    def __init__(self, Variability, RabbitCount=0, MaxRabbitsInWarren=99):
        self.__MAX_RABBITS_IN_WARREN = MaxRabbitsInWarren
        ...
        if self.__RabbitCount == 0:
            self.__RabbitCount = int(
                self.__CalculateRandomValue(
                    int(self.__MAX_RABBITS_IN_WARREN / 4), self.__Variability
                )
            )
    ...
    def __CalculateRandomValue(self, BaseValue, Variability):
        return (
            BaseValue
            - (BaseValue * Variability / 100)
            + (BaseValue * random.randint(0, Variability * 2) / 100)
        )
    ...
    def NeedToCreateNewWarren(self):
        if (
            self.__RabbitCount == self.__MAX_RABBITS_IN_WARREN
            and not self.__AlreadySpread
        ):
    ...
    def AdvanceGeneration(self, ShowDetail):
        ...
        if (
            self.__RabbitCount > 0
            and self.__RabbitCount <= self.__MAX_RABBITS_IN_WARREN
        ):
    ...
    def __MateRabbits(self, ShowDetail):
        ...
        for r in range(0, self.__RabbitCount):
            if (
                self.__Rabbits[r].IsFemale()
                and self.__RabbitCount + Babies < self.__MAX_RABBITS_IN_WARREN
            ):
                Mate = random.randint(0, self.__RabbitCount - 1)
                while Mate == r or self.__Rabbits[Mate].IsFemale():
                    Mate = random.randint(0, self.__RabbitCount - 1)
                CombinedReproductionRate = (
                    self.__Rabbits[r].GetReproductionRate()
                    + self.__Rabbits[Mate].GetReproductionRate()
                ) / 2
                if CombinedReproductionRate >= 1:
                    self.__Rabbits[self.__RabbitCount + Babies] = Rabbit(
                        self.__Variability, CombinedReproductionRate
                    )
                    Babies += 1
...
class GiantWarren(Warren):
    def __init__(self, Variability, RabbitCount=1):
        super().__init__(Variability, RabbitCount, 200)
...
class Animal:
    ...
    def __init__(self, AvgLifespan, AvgProbabilityOfDeathOtherCauses, Variability):
        self._NaturalLifespan = int(
            AvgLifespan * self._CalculateRandomValue(100, Variability) / 100
        )
        self._ProbabilityOfDeathOtherCauses = (
            AvgProbabilityOfDeathOtherCauses
            * self._CalculateRandomValue(100, Variability)
            / 100
        )
        self._IsAlive = True
        self._ID = Animal._ID
...
class Fox(Animal):
    ...
    def __init__(self, Variability):
        ...
        Fox._DEFAULT_LIFE_SPAN = self.__DEFAULT_LIFE_SPAN
        self.__DEFAULT_PROBABILITY_DEATH_OTHER_CAUSES = 0.1
        super().__init__(
            self.__DEFAULT_LIFE_SPAN,
            self.__DEFAULT_PROBABILITY_DEATH_OTHER_CAUSES,
            Variability,
        )
        self.__FoodUnitsNeeded = int(
            10 * self._CalculateRandomValue(100, Variability) / 100
        )
        self.__FoodUnitsConsumedThisPeriod = 0
        self.__Gender = Genders.Male if random.randint(
            1, 3) == 1 else Genders.Female
    ...
    def ReproduceThisPeriod(self):
        ...
        return random.randint(0, 100) < REPRODUCTION_PROBABILITY * 100

    def GiveFood(self, FoodUnits):
        self.__FoodUnitsConsumedThisPeriod += FoodUnits
    ...
    def Inspect(self):
        ...
        print("Gender", self.__Gender.name, end="")
        print()

    @staticmethod
    def getLifeExpect():
        if Fox._TotalDeadFoxes == 0:
            return Fox._DEFAULT_LIFE_SPAN
...
class Rabbit(Animal):
    def __init__(self, Variability, ParentsReproductionRate=1.2, genderRatio=50):
        ...
        self.__ReproductionRate = (
            ParentsReproductionRate *
            self._CalculateRandomValue(100, Variability) / 100
        )
...
class Den:
    def __init__(self):
        self.foxes_created = 0
        self.__time_periods = 0

    def AdvanceGeneration(self, show_details):
        self.__time_periods += 1

    def should_spawn_new_fox(self):
        return self.__time_periods % 3 == 0
